...
void drawPoint(Buffer &buffer, int x, int y, const Color &c) {
	*(buffer.ptr + y * buffer.width + x) = c;
}

void drawLine(Buffer &buffer, int x1, int y1, int x2, int y2, const Color &c) {
...
void drawElement(Buffer &buffer, VertexData& v_array, std::vector<int> &indices, int mode) {
	if (mode == SR_POINT) {
...
public:
	...
	Frag(const VertexData &va, const std::vector<coord> &sp, const int index, int test_mode) {
		...
		_mm_storeu_ps(position.e, _mm_loadu_ps(va.positions[index].e));
		_mm_storeu_ps(normal.e, _mm_loadu_ps(va.normals[index].e));
		
		if (test_mode == TEST_COLOR) {
			_mm_storeu_ps(color.e, _mm_loadu_ps(va.colors[index].e));
		}
		if (test_mode == TEST_LIGHT) {
			_mm_storeu_ps(fragPosInWorld.e, _mm_loadu_ps(va.fragPosInWorlds[index].e));
			_mm_storeu_ps(tangentLightPos.e, _mm_loadu_ps(va.tangentLightPos[index].e));
			_mm_storeu_ps(tangentViewPos.e, _mm_loadu_ps(va.tangentViewPos[index].e));
		}
	...
	void fragmentProgram() {
		...
		if (test_mode == TEST_LIGHT) {
			for (int i = 0; i < fragments.size(); ++i) {
				...
				float specular = pow(max(dot(halfDir, normal), 0.f), 64);
				color *= (ambient + diffuse + specular)*vec3(0.8f);

				/*for (int j = 0; j < lightList.size(); ++j) {
					vec3 lightPos = lightList[j].pos.toVec3();
					vec3 lightDir = (lightPos - fragPos).unit();
					float ambient = 0.1f;
					float diffuse = max(dot(normal, lightDir), 0.f);
					vec3 halfDir = (viewDir + lightDir).unit();
					float specular = pow(max(dot(halfDir, normal), 0.f), 64);
					color += (ambient + diffuse + specular)*lightList[j].color;
				}*/
...
private:
	...
	Frag interpolate(Frag &v0, Frag &v1, Frag &v2, float u, float v) {
		...
		//position
		__m128 result = _mm_mul_ps(_u, _mm_loadu_ps(v1.position.e));
		__m128 _temp = _mm_mul_ps(_v, _mm_loadu_ps(v2.position.e));
		result = _mm_add_ps(result, _temp);
		_temp = _mm_mul_ps(_w, _mm_loadu_ps(v0.position.e));
		_mm_storeu_ps(temp.position.e, _mm_add_ps(result, _temp));

		//normal
		result = _mm_mul_ps(_u, _mm_loadu_ps(v1.normal.e));
		_temp = _mm_mul_ps(_v, _mm_loadu_ps(v2.normal.e));
		result = _mm_add_ps(result, _temp);
		_temp = _mm_mul_ps(_w, _mm_loadu_ps(v0.normal.e));
		_mm_storeu_ps(temp.normal.e, _mm_add_ps(result, _temp));

		if (test_mode == TEST_COLOR) {
			result = _mm_mul_ps(_u, _mm_loadu_ps(v1.color.e));
			_temp = _mm_mul_ps(_v, _mm_loadu_ps(v2.color.e));
			result = _mm_add_ps(result, _temp);
		...
		if (test_mode == TEST_LIGHT) {
			...
			_temp = _mm_mul_ps(_v, _mm_loadu_ps(v2.fragPosInWorld.e));
			result = _mm_add_ps(result, _temp);
			_temp = _mm_mul_ps(_w, _mm_loadu_ps(v0.fragPosInWorld.e));
			_mm_storeu_ps(temp.fragPosInWorld.e, _mm_add_ps(result, _temp));

			result = _mm_mul_ps(_u, _mm_loadu_ps(v1.tangentLightPos.e));
			_temp = _mm_mul_ps(_v, _mm_loadu_ps(v2.tangentLightPos.e));
			result = _mm_add_ps(result, _temp);
			_temp = _mm_mul_ps(_w, _mm_loadu_ps(v0.tangentLightPos.e));
			_mm_storeu_ps(temp.tangentLightPos.e, _mm_add_ps(result, _temp));

			result = _mm_mul_ps(_u, _mm_loadu_ps(v1.tangentViewPos.e));
			_temp = _mm_mul_ps(_v, _mm_loadu_ps(v2.tangentViewPos.e));
			result = _mm_add_ps(result, _temp);
			_temp = _mm_mul_ps(_w, _mm_loadu_ps(v0.tangentViewPos.e));
			_mm_storeu_ps(temp.tangentViewPos.e, _mm_add_ps(result, _temp));
		}
